...
## Team members
- **Thành viên 1**: Sinh vien 1 - MSSV: CHUA_CAP_NHAT
- **Thành viên 2**: Sinh vien 2 - MSSV: CHUA_CAP_NHAT

## Task division
- **Thành viên 1 phụ trách chính**: `sender.py`, quy trinh tao DES key/IV, dong goi packet `key + iv + length + ciphertext`, va log phia gui.
- **Thành viên 2 phụ trách chính**: `receiver.py`, nhan header theo dung thu tu, doc du lieu du so byte, giai ma DES-CBC va log phia nhan.
- **Phần làm chung**: viet test trong `tests/`, chay demo local, phan tich threat model, hoan thien bao cao va peer review.

## Demo roles
- **Bạn nào demo Sender / gói tin / log gửi**: Sinh vien 1
- **Bạn nào demo Receiver / giải mã / log nhận**: Sinh vien 2
- **Cả hai cùng trả lời threat model và ethics**: Ca hai thanh vien
...
Trên giao diện web:
- bật Receiver bằng host/port/timeout mong muốn
- nhập bản tin ở form Sender
- bấm gửi để xem ngay `Key`, `IV`, `Ciphertext` và plaintext phía Receiver
